...
def train_CcLDM(train_images, train_labels, val_images, val_labels, netLDM, save_images_folder, save_models_folder = None):
    ...
    train_dataloader = torch.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=True, num_workers=20, pin_memory=True, prefetch_factor=2)
    
    v_trainset = IMGs_dataset(val_images, val_labels, normalize=True)
    val_dataloader = torch.utils.data.DataLoader(v_trainset, batch_size=v_batch_size, shuffle=True, num_workers=20, pin_memory=True, prefetch_factor=2)
