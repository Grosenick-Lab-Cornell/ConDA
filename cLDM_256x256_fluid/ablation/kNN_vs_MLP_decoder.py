...
import cebra
cebra_model = cebra.CEBRA.load("../cebra/saved_cebra_models/cebra_dim8.pt", weights_only=False)
...
# kNN 
knn_model = cebra.KNNDecoder(n_neighbors=5, metric="euclidean")
...
# ---- 1. MLP Decoder: (8) -> (4096) ---- #

class FluidMLPDecoder(nn.Module):
    def __init__(self,
                 in_dim=8,
                 out_dim=4*32*32,
                 hidden_dims=(256, 256),
...
for cfg in configs:
    ...
    model, (train_hist, val_hist) = train_fluid_decoder(
        C_emb=C_train,
...
test_labels = torch.tensor(labels[idx_test]).type(torch.float).to(device)

inf_steps = 1000
i = list(range(0,len(idx_test),12))
...
X_data = np.load('../data/X_data_Re120_time_3000.npy')[idx_test]
