...
tp = np.load('norm_labels.npy').reshape(88, 100)[:n_Re]
t = tp.reshape(n) # aux variable as time points
...
cebra_model.fit(Z_data, t)
C_train = cebra_model.transform(Z_data)
C_test = cebra_model.transform(shuffled_Z_unseen)
...
X = scaler.fit_transform(C_train)
X_unseen = scaler.transform(C_test)
